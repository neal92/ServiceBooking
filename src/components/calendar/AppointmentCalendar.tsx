...
const AppointmentCalendar: React.FC<AppointmentCalendarProps> = ({ 
  ...
  const [selectedAppointment, setSelectedAppointment] = useState<Appointment | null>(null);  // Charger les rendez-vous
  useEffect(() => {
    loadAppointments();
    
    // Configurer un rechargement périodique (toutes les 30 secondes)
  ...
  const loadAppointments = async () => {
    ...
    try {
      ...
      console.log('Rendez-vous chargés:', data.length);
  ...
  // Ces fonctions ne sont plus nécessaires car nous utilisons le composant NewAppointmentModal
  // qui gère lui-même les opérations de création, mise à jour et suppression
    ...
    <div className="appointment-calendar h-full">
      ...
      {isEditAppointmentModalOpen && selectedAppointment && (
        <NewAppointmentModal
          ...
          appointment={selectedAppointment}          onDelete={async (id: number) => {
            try {
