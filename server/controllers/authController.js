...
exports.register = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Registration attempt for email: ${email}, userType: ${
    ...
    if (!userPseudo) {
      ...
      // Convertir en minuscule et nettoyer le pseudo
      userPseudo = userPseudo.toLowerCase().trim(); // Check if pseudo already exists - C'est cette vérification qui assure qu'il n'y aura pas de doublons
      const [rows] = await db.query("SELECT * FROM users WHERE pseudo = ?", [
        userPseudo,
      ]);
      if (rows.length > 0) {
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    console.log("Profile updated successfully:", {
      id: updatedUser.id,
      avatar: updatedUser.avatar ? "exists" : "null",
    });
  ...
  try {
    ...
    if (!user.password) {
      ...
      return res.status(500).json({
        message: "Erreur serveur: informations utilisateur incomplètes",
      });
...
exports.uploadAvatar = async (req, res) => {
  ...
  try {
    ...
    if (isPredefined) {
      ...
      console.log(
        "uploadAvatar - Utilisation de l'avatar prédéfini:",
        fileName
      );
    } else {
      // Pour les avatars personnalisés
      fileName = `avatar-${req.user.userId}-${Date.now()}.${avatarFile.name
        .split(".")
        .pop()}`;
      ...
      if (!fs.existsSync(uploadsDir)) {
        console.log(
          "uploadAvatar - Création du répertoire uploads:",
          uploadsDir
        );
      ...
      try {
        ...
        console.error(
          "uploadAvatar - Erreur lors du déplacement du fichier:",
          err
        );
        throw err;
      }
    ...
    if (isPredefined) {
      ...
      console.log(
        "uploadAvatar - Utilisation de l'avatar personnalisé:",
        avatarUrl
      );
    }
    try {
      ...
      console.error(
        "uploadAvatar - Erreur lors de la mise à jour de la base de données:",
        err
      );
      throw err;
    }
